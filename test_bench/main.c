...
int main(int argc, char **argv)
{
    int count = 0;
    double count_fp = 0.0;
    // Ici cache_size est initialisé à 4MB car il va nous permettre de remplir les caches entièrement afin d'être certain que array et array_f soit dans la DRAM est plus en cache.
    size_t cache_size = 4 * 1024 * 1024;
    size_t SIZE = atoi(argv[1]);
    int *array = malloc(sizeof(*array) * SIZE);
    double *array_f = malloc(sizeof(*array_f) * SIZE);
    // tmp aura donc un size de 16MB ce qui nous assure de remplir nos 3 caches(L1 32kB, L2 256kB et L3 8MB)
    ...
    LIKWID_MARKER_INIT;
    LIKWID_MARKER_REGISTER("int");
    LIKWID_MARKER_REGISTER("double");
